...
最后我们可以把前四节已有的知识串联起来，完善一下我们的 \lstinline@valarri@ 代码。\par
下面是我修改完善过后的完整代码。相比于之前断断续续写的代码，这里的内容更详细，更具体，还改动了一些查漏补缺代码（比如在构造函数中添加 \lstinline@size@ 超过 \lstinline@MAX_SIZE@ 的检测），新增了 \lstinline@resize@ 和 \lstinline@swap@ 成员函数，对注释部分也做了一些调整。
...
private: //私有成员部分，注意成员变量定义的顺序
    ...
    public:
        ...
        const int* p()const { return _p; } //返回类型是“指向常量的指针”，值为_p
...
\end{lstlisting}
\begin{lstlisting}[caption=\texttt{Definition.h}]
#include "Header.h"
#include <algorithm>
...
valarri::~valarri() {
    ...
    delete[] _arr.p(); //回收_arr.p()指向的动态内存空间，注意必须搭配delete[]
}
